    ...
    func solvePartOne() {
        ...
        func loadData() {
            ...
            rules = components[1]
                ...
                .map { line in
                    createRule(
                        pair: Pair(first: line[line.startIndex], second: line[line.index(line.startIndex, offsetBy: 1)]),
                        target: line[line.index(line.startIndex, offsetBy: 6) ]
                    )
                }
        }
        ...
        func loadData() {
            ...
            rules = components[1]
                ...
                .reduce(into: [Pair: Rule]()) { rules, line in
                    ...
                    let p = Array(components[0]) // character array of the pair
                    let pair = Pair(first: p[0], second: p[1] )
                    let target = Character(components[1])
                    rules[pair] = {
                        var dict = [Pair:Int]()
                        dict[Pair(first: pair.first, second: target), default: 0 ] += polymerPairs[pair]!
                        dict[Pair(first: target, second: pair.second), default: 0] += polymerPairs[pair]!
                        return dict
                    }
                }
        }
